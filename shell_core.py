        ...
        for cmd_index, (command_str, separator) in enumerate(parsed_commands):
            ...
            if separator != Separator.PIPE:
                ...
                if final_command_exec_str: # If there's an actual PowerShell command to execute
                    ...
                    if is_background_job:
                    ...
                    elif can_elevate_this: # True if temp_needs_elevation_overall and not background etc.
                        ...
                        if confirm.lower() == 'y':
                            try:
                                # Get current working directory to pass to the elevated script
                                current_cwd = os.getcwd()
                                
                                # Prepend cd to the command string for the elevated instance
                                # Ensure paths in final_command_exec_str are absolute or correctly relative
                                # to current_cwd if they were originally relative.
                                # However, icacls itself often works better with absolute paths if the CWD changes.
                                # The commands generated by chmod should ideally use absolute paths if possible,
                                # or we ensure the CWD is set.
                                
                                # Option 1: Change directory in the elevated script
                                script_to_elevate = f"Set-Location -LiteralPath '{current_cwd}'; {final_command_exec_str}"
                                
                                # Add a pause to keep the elevated window open
                                ...
                                if elevation_process.returncode != 0:
                                    ui_manager.display_error(f"Failed to initiate elevation: {elevation_process.stderr if elevation_process.stderr else elevation_process.stdout}")
                            ...
                            ui_manager.display_info("Elevation cancelled by user.")
                            last_return_code = 1 
